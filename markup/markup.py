...
class Keyboards:
    ...
    def category_menu(self):
        '''
        создаем разметку кнопок в основном меню category
        '''
        self.markup = ReplyKeyboardMarkup(True, True)
    ...
    def set_select_cathegory(self, category):
        '''
        Создает разметку инлайн кнопок
        :param category: категория, для которой нужно создать список товаров
        ...
        '''
        for el in self.BD.select_all_products_category(category):
            self.markup.add(self.set_inline_btn(el))
